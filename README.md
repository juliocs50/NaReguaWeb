...
- **GitHub Pages:** workflow em `.github/workflows/pages.yml` publica `public/`, copia `index.html` → `404.html` (URLs `/slug` não existem como ficheiro) e copia `CNAME` se existir. O `index.html` define `<base href>` (raiz em domínio próprio; em `*.github.io/repo/` usa `/repo/`). O JS remove o prefixo do repo ao ler o slug (também inferido pelo `app-rest.js`).
